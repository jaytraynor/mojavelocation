...
(function () {
  ...
  function renderThumbs() {
    ...
    state.slides.forEach((s, k) => {
      ...
      const t = elc("button", "thumb", `<img src="${esc(s.src)}" loading="lazy" alt="">`);
  ...
  function showSlide(i) {
    ...
    if (s.type === "img") {
      ...
      g.innerHTML = `<div class="noimg"><div class="camera">\uD83D\uDCF7</div><div>No photo yet</div>` +
        `<div class="hint">Add one in data.js (<code>photos:[…]</code>) or tap “View photos on Google” below.</div></div>`;
    }
