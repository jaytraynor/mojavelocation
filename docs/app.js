...
(function () {
  ...
  /* ---- keyless aerial imagery (Esri World Imagery — no API key needed) ---- */
  const sat = (o, dLat) => {
    if (o.lat == null) return "";
    const dLng = dLat * 2.0;
    return `https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/export?bbox=${o.lng - dLng},${o.lat - dLat},${o.lng + dLng},${o.lat + dLat}&bboxSR=4326&imageSR=4326&size=900,506&format=jpg&f=image`;
  };
  const SAT_LEVELS = [0.0006, 0.0016, 0.0045];
  function aerialSlides(o) {
    if (o.lat == null) return [];
    const names = ["Aerial — close", "Aerial — block", "Aerial — area"];
    return SAT_LEVELS.map((d, i) => ({ type: "img", src: sat(o, d), label: names[i] }));
  }
  function loadPhotos(o) {
    const own = photosOf(o).map((p, i) => ({ type: "img", src: p, label: "Photo " + (i + 1) }));
    let slides = own.concat(aerialSlides(o));
    if (!slides.length) slides = [{ type: "add", n: 1 }];
  ...
  function showSlide(i) {
    ...
    if (s.type === "img") {
      ...
      g.innerHTML = `<div class="noimg"><div class="camera">\uD83D\uDCF7</div><div>No aerial view here</div>` +
        `<div class="hint">If none appear, this spot has no Google photos — tap “View photos on Google” or add to <b>Media</b>.</div></div>`;
    }
  ...
  /* ---- media tab (links saved in your browser) ---- */
  function mediaOf(o) { return MEDIA[idOf(o)] || []; }
  function addMedia(o, url) { url = (url || "").trim(); if (!url) return; const a = mediaOf(o); a.push(url); MEDIA[idOf(o)] = a; store.set("cal_media", MEDIA); renderMedia(o); }
  function removeMedia(o, i) { const a = mediaOf(o); a.splice(i, 1); MEDIA[idOf(o)] = a; store.set("cal_media", MEDIA); renderMedia(o); }
  function mediaEl(url) {
    ...
    if (/\.(jpg|jpeg|png|gif|webp|avif)(\?|$)/.test(u) || u.includes("googleusercontent") || u.includes("ggpht")) return `<img src="${esc(url)}" loading="lazy" alt="">`;
    if (/\.pdf(\?|$)/.test(u)) return `<a class="media-file" href="${esc(url)}" target="_blank" rel="noopener">\uD83D\uDCC4 Open PDF</a>`;
    return `<a class="media-file" href="${esc(url)}" target="_blank" rel="noopener">\uD83D\uDD17 ${esc(url.replace(/^https?:\/\//, "").slice(0, 44))}</a>`;
  }
  function renderMedia(o) {
    const box = $("#media-list"); const a = mediaOf(o);
    if (!a.length) { box.innerHTML = '<div class="media-empty">No media yet. Paste a link above — photo, video, PDF, YouTube, Vimeo, or Google Drive.</div>'; return; }
    box.innerHTML = "";
    a.forEach((url, i) => { box.appendChild(elc("div", "media-cell", mediaEl(url) + `<button class="media-x" data-i="${i}" title="Remove">\u00D7</button>`)); });
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    $("#media-add-btn").addEventListener("click", () => { const inp = $("#media-url"); if (state.current) { addMedia(state.current, inp.value); inp.value = ""; } });
    $("#media-url").addEventListener("keydown", (e) => { if (e.key === "Enter" && state.current) { const inp = $("#media-url"); addMedia(state.current, inp.value); inp.value = ""; } });
    // lightbox
