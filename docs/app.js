...
(function () {
  ...
  function sceneCount(id) {
    return OPTIONS.filter(o => o.region === state.region && (!state.area || o.area === state.area) && o.scenes && o.scenes.indexOf(id) !== -1).length;
  }
  ...
  function buildSceneSelect() {
    ...
    const total = filteredNoScene();
    sel.appendChild(new Option(`All locations (${total})`, "all"));
    (CONFIG.scenes || []).forEach(sc => {
      const n = sceneCount(sc.id);
      sel.appendChild(new Option(`${sc.name} (${n})`, sc.id));
    });
    sel.value = state.scene;
  }
  function filteredNoScene() {
    return OPTIONS.filter(o => o.region === state.region && (!state.area || o.area === state.area)).length;
  }
    ...
    if (!state.list.length) {
      box.appendChild(elc("div", "empty",
        state.scene === "all" ? "No locations here yet.<br>Add them in <b>data.js</b>."
          : `No options tagged for <b>${esc(sceneById(state.scene) ? sceneById(state.scene).name : state.scene)}</b> yet.`));
      return;
    }
  ...
  function renderThumbs() {
    ...
    th.style.display = th.children.length > 1 ? "flex" : "none";
  }
  function showSlide(i) {
    ...
    if (s.type === "img") {
      ...
      g.innerHTML = `<div class="noimg"><div class="camera">\uD83D\uDCF7</div><div>Loading photo…</div>` +
        `<div class="hint">${KEY ? "If nothing appears, there's no Street View here — use the buttons below." : "Use the buttons below to see photos."}</div></div>`;
    }
  ...
  function openModal(o) {
    ...
    state.slides = photosOf(o).map(p => ({ type: "img", src: p, label: "Photo" }));
    if (!state.slides.length) state.slides = [{ type: "placeholder" }];
    renderThumbs(); showSlide(0);
    $("#modal").hidden = false;
    addStreetView(o);
  }
  function addStreetView(o) {
    if (!KEY || o.lat == null) return;
    fetch(svMeta(o)).then(r => r.json()).then(j => {
      if (state.current !== o || !j || j.status !== "OK") return;
      const hadPlaceholder = state.slides.length === 1 && state.slides[0].type === "placeholder";
      const slide = { type: "img", src: svStatic(o), label: "Street View" };
      if (hadPlaceholder) { state.slides = [slide]; renderThumbs(); showSlide(0); }
      else { state.slides.push(slide); renderThumbs(); }
    }).catch(() => { });
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    $("#foot").innerHTML = KEY
      ? "Photos load from Street View automatically. Use “Show” to filter by scene."
      : "Add a Google Maps key in data.js for auto photos. Use “Show” to filter by scene.";
  });
})();
