...
(function () {
  ...
  function previewShow(o) {
    const box = $("#preview");
    const imgs = photosOf(o).slice(0, 3).map(p => `<img src="${esc(p)}" loading="lazy" alt="">`);
    if (KEY && o.lat != null) imgs.push(`<img src="${svStatic(o)}" loading="lazy" alt="" onerror="this.style.display='none'">`);
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    $("#foot").innerHTML = "Hover a location to preview photos · click a photo to zoom (space/esc closes) · \u2605 favorites & status save in your browser.";
  });
})();
